...
This code is specificly for DE method for PCRM.
...
n=int(totaln/2)# 750 for IS method to generate quantile and mean estimators, 750 for SRS method to generate quantile and mean estimators
n1=n
n2=n
batch=10
...
#see paper G.3. Two-step IS to Estimate Quantile for details
...
#see paper G.2.2. Applying Two-Step IS to Estimate P(Y > x) for more details
    Z=x_ini
...
def derivative_cgf(theta):#return derivative of conditional cgf
    ...
    sumlist=np.sum(akbk)
    return sumlist  
def theta_rootfinder(theta):#this function is for finding root theta in G.2.2. Applying Two-Step IS to Estimate P(Y > x)
...
eta_test1=[0]*replication
eta_test2=[0]*replication

quantile_test1=[0]*replication
quantile_test2=[0]*replication

mean_test1=[0]*replication
mean_test2=[0]*replication
for r in range(0,replication):  
    #this loop next is for getting crude quantile approximation
    for r_QA in range(0,rx):
    ...
    for i in range(0,batch):
        ...
        quantile_estimator1=LR[k][0]
        
        mean_estimator1=np.dot(L,likelihood)/len(L)
        
        for j in range(0,m2):#this for loop use SRS to generate quantile and mean estimators
            ...
            L_SRS_B[cclb2]=l_SRS
            cclb2=cclb2+1
        mean_estimator2=np.mean(L_SRS)
        
        mean_estimator[i]=(mean_estimator1+mean_estimator2)/2
        
        L_SRS=sorted(L_SRS)
        s=math.ceil(m2*pquantile)-1
        
        quantile_estimator2=L_SRS[s]
        
        quantile_estimator[i]=(quantile_estimator1+quantile_estimator2)/2
    ...
    L_SRS_B=sorted(L_SRS_B)
    s=math.ceil(n2*pquantile)-1
    quantile_n2=L_SRS_B[s]
    
    quantile_n=(quantile_n1+quantile_n2)/2
    
    quantile_test1[r]=quantile_n1
    quantile_test2[r]=quantile_n2
    
    mean_test1[r]=np.dot(LB,Blikelihood)/len(LB)
    mean_test2[r]=np.mean(L_SRS_B)
    
    eta_test1[r]=quantile_test1[r]-mean_test1[r]
    eta_test2[r]=quantile_test2[r]-mean_test2[r]
    ...
    HW_s_eta[r]=t_value*math.sqrt(eta_var/batch)/eta_n
    ...
    HW_b_eta[r]=t_value*math.sqrt(eta_var/batch)/eta_n
