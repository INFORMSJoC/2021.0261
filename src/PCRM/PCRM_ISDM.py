...
This code is specificly for MSIS method for ISDM.
...
n2=n
batch=10
m1=int(n/batch)# there will be 10 groups,  m1=150 samples/group
m2=m1
print('m',m)
print('d',d)
...
def mgf(theta,pkz):#conditional moment generating funciton
    if theta==0:
        return 1
    pk=np.array(pkz)
    bet=np.array(beta)
...
def derivative_cgf(theta,pkz):#return derivative of conditional cgf
    if theta==0:
    ...
    sumlist=np.sum(akbk)
    return sumlist  
def theta_rootfinder(theta):#this function is for finding root theta. Applying Two-Step IS to Estimate P(Y > x)
    return derivative_cgf(theta,pkz)-x
...
whole=replication*n1
Lwhole=[0]*whole
wholelikelihood=[0]*whole
cc_whole=0
...
for r in range(0,replication): 
    #this loop  is for getting crude quantile approximation
    for r_QA in range(0,rx):
        x=x_quantile[r_QA]
        mu=minimize(optimal_z,x0=np.random.uniform(0,5,d),method='COBYLA')["x"]
        
        
        for jj in range(0,n_QA):
          
            Z=np.random.normal(loc = mu, scale = 1, size =d)  #generate Z with mean mu
            pkz=norm.cdf((np.dot(A,Z)+inv_p)/B)
            E=np.dot(pkz,beta)/2
            ...
            if  E>=x:
                ...
                a=0
                if derivative_cgf(a,pkz)-x>=0:
                    b=-0.1
                    while(derivative_cgf(b,pkz)-x>0):
                        b=2*b
                else:
                    b=0.1
                    while(derivative_cgf(b,pkz)-x<=0):
            ...
            rk=pkz if theta==0 else 1/((1-pkz)*theta*beta/(pkz*(np.exp(theta*beta)-1))+1)
            ...
            l=np.dot(Y,C)
            L_QA[jj]=l
            likeli=math.exp(-theta*l)*mgf(theta,pkz)*math.exp(-np.dot(mu,Z)+np.dot(mu,mu)/2)
            likelihood_QA[jj]=likeli
    ...
    for i in range(0,batch):
        for j in range(0,m1):  
            uiform=random.uniform(0,1)
            if uiform<delta_0:
                
                Z=np.random.normal(loc = setmu, scale = 1, size =d)
                ...
                LB[cclb1]=l
                Lwhole[cc_whole]=l
                likeli_deno=math.exp(-theta*l)*mgf(theta,pkz)*math.exp(-np.dot(setmu,Z)+np.dot(setmu,setmu)/2)
            ...
            likeli=1/(delta_0/likeli_deno+1-delta_0)
           
            likelihood[j]=likeli
            Blikelihood[cclb1]=likeli
            cclb1=cclb1+1
        
            wholelikelihood[cc_whole]=likeli
            cc_whole=cc_whole+1
