...
This code is specificly for IS_xi method for PCRM.
...
#so in total sample size is 1500+500=2000
n=totaln
n1=n
n2=n
batch=10
...
#print x_quantile to check if the given "true" quantile is in the interval
...
for r in range(0,replication):   
    #this loop below is for getting crude quantile approximation
    ...
    join_mu_fun=[[minimize(optimal_z,x0=[random.uniform(-1,0) for k in range(d)],method='COBYLA')[k] for k in my_keys] for i in range(ini_point_numbers)]
    ...
    CI_s_1=eta_n-t_value*math.sqrt(eta_var/batch)
    CI_s_2=eta_n+t_value*math.sqrt(eta_var/batch)
    CI_b_eta[r]=[CI_s_1,CI_s_2]#save CI
