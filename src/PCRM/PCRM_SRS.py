...
This code is specificly for MSIS method for SRS.
...
#x=20
replication=1000#this is the number of replications we need in total
...
for r in range(0,replication):
    ...
    for i in range(0,batch):
        for j in range(0,m1): 
            ...
            X=np.dot(A,Z)+B*epsilon#this is the a_k*Z+b_k*epsilon in the model 
            
            Y=np.greater(X,x)*1# this is the I function in the model (note that we let S=1, so we don't need to include it here
            ...
            l=sum(C)#this is c(X) in the model
